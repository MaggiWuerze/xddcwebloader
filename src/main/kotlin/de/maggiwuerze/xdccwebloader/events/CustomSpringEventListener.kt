...
class CustomSpringEventListener(
    ...
    fun onDownloadCreationEvent(event: DownloadCreationEvent) {
        val download: Download = downloadService.getById(event.payload)
    ...
    fun onDownloadUpdateEvent(event: DownloadUpdateEvent) {
        downloadService.update(downloadService.getById(event.payload))
        eventPublisher.sendWebsocketEvent(SocketEvents.UPDATED_DOWNLOAD, downloadService.getById(event.payload))
    }

    @EventListener
    fun onDownloadDeleteEvent(event: DownloadDeleteEvent) {
        eventPublisher.sendWebsocketEvent(SocketEvents.DELETED_DOWNLOAD, downloadService.getById(event.payload))
